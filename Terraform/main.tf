...
resource "launchdarkly_project" "does" {
  key  = "ld-demo-does"
  name = "ld-demo-does"
...
resource "launchdarkly_feature_flag" "qrcode" {
  project_key = launchdarkly_project.does.key
  key         = "qrcode"
  name        = "0 - QR Code"
...
output "LaunchDarkly_API_Key" {
  value = launchdarkly_project.does.environments[0].api_key
  sensitive = true
}

output "LaunchDarkly_Client_Side_Key" {
  value = launchdarkly_project.does.environments[0].client_side_id
  sensitive = true
}
